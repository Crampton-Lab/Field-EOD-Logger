// CODE WRITTEN BY STEFAN MUCHA https://github.com/muchaste/EOD-logger 
// MODIFIED HERE FOR 2-CHANNEL ACQUISITION WITH CONSECUTIVE 60-SECOND RECORDINGS AT 49.152 KSS SAMPLING RATE
...
//char filename[50];
//char theTime[50];
//setup start and end timer vars
byte currentHourS;
byte currentMinuteS;
byte currentSecondS;
...
////////////start time vars////////////////////
byte starthourTimer = 16;
byte startminuteTimer = 30;
byte startsecondTimer = 0;
///////////////////////////////////////////////

////////////end time vars/////////////////////
uint16_t filesSaved = 2000;                                 //this var stops recording once weve aquired wanted # of files
//////////////////////////////////////////////


/////////////How many channels? 2(default) or 4?/////////////////
//**--------------------------------------------
//initially define your numOfChannels
#define numOfChannels 2     //<------CHANGE THIS TO 2 OR 4 CHANNELS!!
//**--------------------------------------------
...
uint16_t ChannelsCfg_0 [] =  { 0x4E};                              //ADC0: A1   ADC0_SE14= 01110,E     0x4E
uint16_t ChannelsCfg_1 [] =  { 0x44};                              //ADC1: A16  ADC1_SE4b= 00100,4     0x44
...
const uint32_t adc_freq = 49152;                                                     //Sampling rate for one ADC each pin on the ADC shares this frequency e.g. if 4 pins are used adc_freq/4
...
volatile int dma_partitions = 2;                                                      //How often do you want to write
...
const int     times_buffer    = 360;                               // creation of a file based on a multiple of the buffer size
...
#elif numOfChannels==4
//num=4 setup
/*DECLARATIONS ADC and Pins---------------------------------------*/
ADC *adc = new ADC();                                                                 //used to declare the adc->### object

uint16_t ChannelsCfg_0 [] =  { 0x4E, 0x46};                              //ADC0: A1,A6   A1=ADC0_SE14= 01110,E     0x4E
uint16_t ChannelsCfg_1 [] =  { 0x44, 0x45};                              //ADC1: A16, A17  ADC1_SE4b= 00100,4     0x44
const uint16_t ChannelPinNumber0 = sizeof(ChannelsCfg_0) / sizeof(ChannelsCfg_0[0]);  //Needed for reordering
const uint16_t ChannelPinNumber1 = sizeof(ChannelsCfg_1) / sizeof(ChannelsCfg_1[0]);  //Both need to be equal, because PDB gets intialized simultanously for ADC0 and ADC1

const uint32_t adc_freq = 98304;                                                     //Sampling rate for one ADC each pin on the ADC shares this frequency e.g. if 4 pins are used adc_freq/4

volatile int buffer_dma0_position = 0;                                                //Pointer position in DMA Buffer, used with partition to determine where to write
volatile int buffer_dma1_position = 0;
volatile int dma_partitions = 4;                                                      //How often do you want to write
volatile int partition = BUF_DIM / dma_partitions;
volatile int partition1 = BUF_DIM / dma_partitions;
const int partition_sample_amount = BUF_DIM / dma_partitions;
...
uint32_t      FILE_SIZE       = 0;                                    // filesize DMA-Buffer*times_buffer
uint32_t      last            = 0;                                    // coutner uint16_t values that are written to SD card
const int     times_buffer    = 720;                               // creation of a file based on a multiple of the buffer size
...
void setup()
{
  /*RTC sync,issues with current code's RTC------------------------------------------------------*/
  bool parse = false;
  bool config = false;
  ...
  /*Quick Blink LED test pin for making sure its sync'd------------------------------------------------------*/
#ifdef DEBUG
  ...
  Serial.begin(115200);
  delay(5000);
  Serial.print("numOfChannels=");
  Serial.println(numOfChannels);
  delay(5000);
...
void loop() {
...
#endif
    //compare if count and timer equal each other,if they do turn begRecord to true
    if ((currentHourS == starthourTimer) && (currentMinuteS >= startminuteTimer) && (currentSecondS >= startsecondTimer)) {
      begRecord = 1;
      setupFile();
      setupADC();
      setupWAVHeader();
  ...
  if (begRecord == 1) {
    ...
    if ( last >= FILE_SIZE ) {                                                // check if end of file is reached
      ...
      last = 0;                                                               // reset last
#ifdef DEBUG
      digitalWriteFast(LED_BUILTIN, HIGH);                                    // turn on LED
#endif
      createNewFile();                                                        // create new files for data logging
    }

    if (fileNr >= filesSaved) {
      //stop recording to SD and shut it down
      begRecord = 2;
    }
...
void createNewFile() {
  ...
  char filename[50];
  snprintf(filename, 50, "%d%02d%02d-%02d%02d%02d.wav", y, mo, d, h, m, s);
...
#ifdef DEBUG
  digitalWriteFast(LED_BUILTIN, LOW);                                    // turn off LED
#endif
}
